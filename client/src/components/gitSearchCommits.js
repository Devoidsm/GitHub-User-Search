...
const SearchgitCommits= () => {
	const [searchedRepo, setCommits] = React.useState('');
	const { searchRepoCommits } = React.useContext(GitInfo);
	//event that handles after form  is sumbitted
	const handleFormSubit = (event) => {
		event.preventDefault();
		if (searchedRepo) {
			searchRepoCommits(searchedRepo);
		}
				...
				<form onSubmit={handleFormSubit}>
					<div className='form-control'>
						...
						<input
							...
							type='text'
							placeholder='Please enter github Repo you wish to see commits for'
							value={searchedRepo}
							onChange={(e) => {
...
`;
export default SearchgitCommits;